  ...
  <div class="table-operator">
    ...
    >
      <span slot="action" slot-scope="text, record">
        <template>
          <a @click="deleteMember(record)">{{ $t('menu.project.view.action.delete') }}</a>
        </template>
      </span>
...
import { getRoleMember, addRoleMember, deleteRoleMember } from '@/api/permission'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      selectUserID: '',
      loadData: parameter => {
        return getRoleMember({ role_id: this.roleID }).then(res => {
            return res.result
        })
      }
    }
  ...
  methods: {
    ...
    handleAdd () {
        ...
        addRoleMember(addUser).then(res => {
          this.selectUserID = ''
          this.$refs.roleMemberTable.refresh()
        })
    },
    handleSelect (value) {
    ...
    deleteMember (record) {
        deleteRoleMember(record).then(res => {
          this.$refs.roleMemberTable.refresh()
        })
    }
  }
