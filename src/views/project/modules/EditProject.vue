<template>
  <a-spin :spinning="loading">
    <page-header-wrapper>
      <a-card :title="$t('permissionid.edit.project')" :headStyle="{fontWeight:'bold'}" :bodyStyle="{padding:'30px 30px'}">
...
import { getProjectSerial, editProject } from '@/api/project'
...
export default {
  ...
  created () {
    ...
    this.editProject(this.$route.params)
  },
  methods: {
    ...
    editProject (data) {
      if (data.ex_open_date.indexOf('1970-01-01') >= 0) {
        data.ex_open_date = ''
      }
      if (data.ex_close_date.indexOf('1970-01-01') >= 0) {
        data.ex_close_date = ''
      }
      fields.forEach(v => this.form.getFieldDecorator(v))
