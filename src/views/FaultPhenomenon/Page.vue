      ...
      >
        <a-form class="permission-form" :form="form">
          <a-row :gutter="24">
            <a-col :span="24">
              <a-form-item :label="$t('menu.fault.phenomenon')">
                <a-input v-decorator="['fault', { rules: [{required: true, message: $t('menu.fault.phenomenon')}] }]" />
              </a-form-item>
            </a-col>
            <a-col :span="24">
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getFaultPhenomenonList(requestParameters)
        .then(res => {
            return res.result
        })
      },
  ...
  methods: {
    ...
    handleEdit (record) {
      ...
      const faultFields = ['id', 'date', 'fault', 'memo']
