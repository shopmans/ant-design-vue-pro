...
import { getStepUsers, addExecutorUsers, delExecutorUsers, getExecutorUserList } from '@/api/step'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return getExecutorUserList({ flow_id: this.flowID, current_step: this.currentStep, flag: this.flag }).then(res => {
            return res.result
        })
      }
    }
  ...
  methods: {
    ...
    handleAdd () {
      ...
      addExecutorUsers(addUser).then(res => {
    ...
    deleteStep (record) {
        delExecutorUsers(record).then(res => {
            this.$refs.stepUserTable.refresh()
        })
    }
  },
